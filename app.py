...
# ---- Temporary in-memory "database" — swap for DynamoDB on Day 3 ----
users_store = {}
items_store = []
...
def api_login():
    ...
    stored_hash = users_store.get(username)
...
def get_contact(item_id):
    ...
    return jsonify({'contact': item['contact']})
...
def create_item():
    ...
    form = request.form
    required = ['title', 'type', 'category', 'location', 'date', 'description', 'contact']
